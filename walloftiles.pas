...
// If you need any help or want to use it in real app please let me know.
// Adjusted by zittergie for XiXMusicPlayer
{$mode objfpc}{$H+}
...
uses
  ...
  Math, contnrs, ExtCtrls, Forms, AnimateEasing, LCLIntf, BCTypes, Dialogs;
...
begin
  ...
  case ChangeEffect of
    tceEffect1: bmp := Effect1(DrawTiles.Items[ChangeSource].Bitmap, ChangeDestinationBitmap, Value, $00E2A366);
    tceEffect2: bmp := Effect2(DrawTiles.Items[ChangeSource].Bitmap, ChangeDestinationBitmap, Value, $00E2A366);
    tceEffect3: bmp := Effect3(DrawTiles.Items[ChangeSource].Bitmap, ChangeDestinationBitmap, Value, $00E2A366);
  end;
...
begin
  ...
  SetLength(inl, nc);
  for i := 0 to Length(inl) - 1 do inl[i] := i mod Tiles.Count;
  if Assigned(Wall) then FreeAndNil(Wall);
  Wall := TBGRABitmap.Create(nw * ts, nh * ts, BGRABlack);

  if Assigned(EffectedWall) then FreeAndNil(EffectedWall);

  //ttt := GetTickCount64;
  EffectedWall := WallEffect1(Wall.Width, Wall.Height, 100);
  //WriteLn(GetTickCount64 - ttt);

  if Assigned(DrawTiles) then FreeAndNil(DrawTiles);
  ...
  begin
    if cd <= x4c then xs := 4
                 else if cd <= x4c + x3c then xs := 3
                                         else xs := 1;

    j := Random(nc);
    if SpaceAvailable(j, xs) then AddTile(j, xs)
                             else if xs > 1 then
                                            begin
                                              xs := 1;
                                              if SpaceAvailable(j, xs) then AddTile(j, xs);
...
begin
  ...
  begin
   ...
   // FormCoverPlayer.ImageCDCover.Picture.Bitmap:=ChangeDestinationBitmap.Bitmap;
...
begin
  ...
  if Active then Bitmap.PutImage(BitmapX, BitmapY, Wall, dmSet)
            else Bitmap.Fill(BGRAWhite);

  if Assigned(FOnDrawWall) then FOnDrawWall(Self, Bitmap);
end;
...
begin
    ...
    if BlackAndWhite then Result.InplaceGrayscale
                     else if BlackAndWhiteAndColor then
                                                   begin
                                                     x:=round(random(50));
                                                     if x>25 then Result.InplaceGrayscale;
                                                   end;
end;
