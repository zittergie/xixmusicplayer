...
{                                                                             }
{ From Version 1.7 on mantained by Bart Dezitter                              }
{ E-mail: info@xixmusicplayer.org                                             }
{ http://www.xixmusicplayer.org                                               }
{                                                                             }
{ Version 1.7 (23 December 2013)                                              }
...
type
  ...
  TOggVorbis = class(TObject)
    ...
    public
      ...
      property OrigArtist: string read FOrigArtist write FOrigArtist;  { Original Artist string }
...
begin
  ...
  begin
    ...
    FOrigArtist := Info.Tag.FieldData[8];
...
begin
  ...
  begin
    ...
    Info.Tag.FieldData[8] := Trim(FOrigArtist);
   ...
   { Info.Tag.FieldData[1] := Trim(Title);  ShowMessage(Title); Showmessage(Filename);
    Info.Tag.FieldData[2] := Trim(Artist);
    Info.Tag.FieldData[3] := Trim(Album);
    if Track > 0 then Info.Tag.FieldData[4] := IntToStr(Track)
    else Info.Tag.FieldData[4] := '';
    Info.Tag.FieldData[5] := Trim(Date);
    Info.Tag.FieldData[6] := Trim(Genre);
    Info.Tag.FieldData[7] := Trim(Comment);
    Info.Tag.FieldData[8] := Trim(OrigArtist);
    Info.Tag.FieldData[9] := '';
    Info.Tag.FieldData[10] := Trim(Encoder);
    Info.Tag.FieldData[11] := Trim(Composer);
    Info.Tag.FieldData[12] := Trim(Copyright);
    Info.Tag.FieldData[13] := Trim(License);
    Info.Tag.FieldData[14] := Trim(EncodedBy);   }
    Tag := BuildTag(Info);
