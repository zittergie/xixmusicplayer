...
type
    ...
    public
       ...
       // UTF8Encoding: TEncoding;
        constructor Create;
        destructor Destroy; override;
        function LoadFromFile(const FileName2: String): Integer;
        function SaveToFile(const FileName2: String): Integer;
...
begin
    Result :='';
    for i:=0 to length(Value)-1 do Result := Result+chr(Value[i]);
...
begin
    ...
    for i := 0 to List.Count - 1 do begin
        BytesName := StrToBytes(List.Names[i]);
        BytesValue := StrToBytes(List.ValueFromIndex[i]);
...
function TFlacTag.LoadFromFile(const FileName2: String): Integer;
begin
    Clear;
...
begin
    ...
    with FlacTagCoverArtInfo do begin
        ...
        Bytes := StrToBytes(MIMEType);
        Stream.Write(Bytes[0], Length(Bytes));
        Bytes := StrToBytes(Description);
...
function TFlacTag.SaveToFile(const FileName2: String): Integer;
var
...
begin
    ...
    try
        ...
        if GetInfo(FileName, False) <> FLACTAGLIBRARY_SUCCESS then begin
...
begin
    ...
    try
        ...
        if (FPadding <> iNewPadding) or bRearange then begin // fill the block with zeros
            Data := 0;
            for k := 0 to iNewPadding - 1 do begin
        ...
        if bRebuild then begin // time to put back the audio data...
            ...
            Source.Free;
            Destination.Free;
            if (DeleteFile(FileName)) AND (RenameFile(BufferName, FileName)) then begin //Replace old file and delete temporary file
                Result := FLACTAGLIBRARY_SUCCESS;
            end else begin
                Result := FLACTAGLIBRARY_SUCCESS;
