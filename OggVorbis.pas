...
uses
  Classes, SysUtils, lazutf8classes, fileutil, LazFileUtils;
...
type
  ...
  TOggVorbis = class(TObject)
    ...
    public
      ...
      property OriginalTitle: string read FOriginalTitle write FOriginalTitle; { Original title }
      ...
      property Orchestra: string read FOrchestra write FOrchestra;        { License/URL string }
      property Conductor: string read FConductor write FConductor;        { License/URL string }
      ...
      property OriginalDate: string read FOriginalDate write FOriginalDate;        { License/URL string }
...
function GetID3v2Size(const Source: TFileStreamUTF8): Integer;
type
  ID3v2Header = record
...
procedure ReadTag(const Source: TFileStreamUTF8; var Info: FileInfo);
var
...
procedure SetCRC(const Destination: TFileStreamUTF8; Info: FileInfo);
var
  Index: Integer;
  Value: Cardinal;
...
begin
  ...
  Result := false;
  if (not FileExistsUTF8(FileName)) { or (FileSetAttr(FileName, 0) <> True} then exit;
  try
    { Create file streams }
    BufferName := FileName + '~';
    Source := TFileStreamUTF8.Create(FileName, fmOpenRead);
