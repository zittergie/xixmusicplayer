...
uses
  Classes, SysUtils, ID3v1, ID3v2, APEtag, lazutf8classes;
...
const
  ...
  AAC_HEADER_TYPE_MP42 = 3;                                           { MPEG4 }
  AAC_HEADER_TYPE_M4A = 4;                                           { MPEG4 }

  { Header type names }
  AAC_HEADER_TYPE: array [0..4] of string =
    ('Unknown', 'ADIF', 'ADTS', 'MP42', 'M4A');
  ...
  { FileFormat codes - MPEG4VersionID}
  MP4_FILEFORMAT_UNKNOWN = 0;                                          { Unknown }
  MP4_FILEFORMAT_1 = 1;                                          { Unknown }
  MP4_FILEFORMAT_2 = 2;                                          { Unknown }
  MP4_FILEFORMAT_M4A = 3;                                          { Unknown }


  { MPEG-4 Fileformat}
  MPEG4_FILEFORMAT_TYPE: array [0..3] of string =
    ('Unknown', 'MP4 file format version 1','MP4 File format version 2', 'M4A File Format');
...
type
  ...
  TAACfile = class(TObject)
    private
      ...
      function FRecognizeHeaderType(const Source: TFileStreamUTF8): Byte;
      procedure FReadADIF(const Source: TFileStreamUTF8);
      procedure FReadADTS(const Source: TFileStreamUTF8);
      procedure FReadMP42(const Source: TFileStreamUTF8);
      procedure FReadM4A(const Source: TFileStreamUTF8);
    ...
    public
      ...
      property MPEG4VersionID: Byte read FMPEGVersionID;   { MPEG version code }
...
function ReadBits(Source: TFileStreamUTF8; Position, Count: Integer): Integer;
var
...
var Type_Version: String;
begin
  { Get MPEG version name }
  Type_Version:=AAC_MPEG_VERSION[FMPEGVersionID];
  if FMPEGVersionID=2 then Type_Version:=Type_Version+', '+MPEG4_FILEFORMAT_TYPE[FMPEG4VersionID];
...
function TAACfile.FRecognizeHeaderType(const Source: TFileStreamUTF8): Byte;
var
  Header: array [1..12] of Char;
  Head4, Head8, head12: string;
begin
  ...
  Head4:=Header[1] + Header[2] + Header[3] + Header[4];
  Head8:=Header[5] + Header[6] + Header[7] + Header[8];
  Head12:=Header[9] + Header[10] + Header[11] + Header[12];
  if Head4 = 'ADIF' then Result := AAC_HEADER_TYPE_ADIF
    else if (Byte(Header[1]) = $FF) and (Byte(Header[1]) and $F0 = $F0) then
         Result := AAC_HEADER_TYPE_ADTS
    else if Head8='ftyp' then
         begin
           if Head12='mp42' then Result:=AAC_HEADER_TYPE_MP42;
           if Head12='M4A ' then Result:=AAC_HEADER_TYPE_M4A;
         end;
end;
...
procedure TAACfile.FReadM4A(const Source: TFileStreamUTF8);
var
  Position: Integer;
begin
  ...
  FMPEG4VersionID := MP4_FILEFORMAT_M4A;
...
procedure TAACfile.FReadMP42(const Source: TFileStreamUTF8);
var
  Position: Integer;
begin
  { Read ADIF header data }
  ...
  FMPEGVersionID := AAC_MPEG_VERSION_4;
  FMPEG4VersionID := MP4_FILEFORMAT_2;
  FProfileID := ReadBits(Source, Position, 2) + 1;
  Inc(Position, 2);
  FSampleRate := SAMPLE_RATE[ReadBits(Source, Position, 4)];
  Inc(Position, 4);
...
procedure TAACfile.FReadADIF(const Source: TFileStreamUTF8);
var
  Position: Integer;
begin
  { Read ADIF header data }
...
procedure TAACfile.FReadADTS(const Source: TFileStreamUTF8);
var
...
begin
  ...
  if (FID3v2.ReadFromFile(FileName)) and (FID3v1.ReadFromFile(FileName)) and (FAPEtag.ReadFromFile(FileName)) then
    try
      SourceFile := TFileStreamUTF8.Create(FileName, fmOpenRead or fmShareDenyWrite);
      ...
      if FHeaderTypeID = AAC_HEADER_TYPE_MP42 then FReadMP42(SourceFile);
      if FHeaderTypeID = AAC_HEADER_TYPE_M4A then FReadM4A(SourceFile);
      SourceFile.Free;
      Result := true;
    except
    end;
